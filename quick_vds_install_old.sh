...
install_php() {
    ...
    # Add PHP repository with Ubuntu 24.04 fallback
    local ubuntu_codename=$(lsb_release -sc)
    log_debug "Adding PHP repository for $ubuntu_codename..."
    
    # Ubuntu 24.04 Noble has PHP 8.3 by default, which is compatible
    if [[ "$ubuntu_codename" == "noble" ]]; then
        log_info "Ubuntu 24.04 detected - using native PHP 8.3 (Laravel compatible)"
        
        # Update package lists for native packages
        retry_command $MAX_RETRIES $RETRY_DELAY "apt update"
        
        # Set PHP version for package installation
        PHP_VERSION="8.3"
    else
        log_info "Adding Ondrej PHP repository for $ubuntu_codename..."
